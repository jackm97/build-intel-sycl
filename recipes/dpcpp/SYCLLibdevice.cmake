...
set(compile_opts
    ...
    -Wno-sycl-strict
    # Disable warnings for the host compilation, where we declare all functions
    # as 'static'.
...
# For NVPTX and AMDGCN each device libary is compiled into a single bitcode file
# and all files created this way are linked into one large bitcode library.
...
#
# Arguments: * FILETYPE <string> Specifies the output file type of the
# compilation and its repsective installation directory. Adds a new target that
# the libsycldevice-FILETYPE target will depend on. * SRC <string> ... Source
# code files needed for the compilation. * EXTRA_OPTS <string> ... List of extra
# compiler options to use. Note that the ones specified by the compile_opts var
# are always used. * DEPENDENCIES <string> ... List of extra dependencies to
# inject
#
...
#
# Arguments: * PROPERTY_NAME <string> The name of the property to append to.
function(append_to_property list)
  cmake_parse_arguments(ARG "" "PROPERTY_NAME" "" ${ARGN})
...
# Creates device libaries for all filetypes. Adds bitcode library files
# additionally for each devicelib_arch target and adds the created file to an
# arch specific global property.
#
# Arguments: * SRC <string> ... Source code files needed for the compilation. *
# EXTRA_OPTS <string> ... List of extra compiler options to use. Note that the
# ones specified by the compile_opts var are always used. * DEPENDENCIES
# <string> ... List of extra dependencies to inject
#
# Depends on the clang target for compiling.
function(add_devicelibs filename)
  cmake_parse_arguments(ARG "" "" "SRC;EXTRA_OPTS;DEPENDENCIES" ${ARGN})
...
if("native_cpu" IN_LIST SYCL_ENABLE_BACKENDS)
  if(NOT DEFINED NATIVE_CPU_DIR)
    message(
      FATAL_ERROR
        "Undefined UR variable NATIVE_CPU_DIR. The name may have changed.")
  endif()
  # Include NativeCPU UR adapter path to enable finding header file with state
  # struct. libsycl-nativecpu_utils is only needed as BC file by NativeCPU.
...
# ones.
add_devicelibs(libsycl-itt-stubs SRC itt_stubs.cpp DEPENDENCIES ${itt_obj_deps})
add_devicelibs(libsycl-itt-compiler-wrappers SRC itt_compiler_wrappers.cpp
               DEPENDENCIES ${itt_obj_deps})
add_devicelibs(libsycl-itt-user-wrappers SRC itt_user_wrappers.cpp DEPENDENCIES
               ${itt_obj_deps})

add_devicelibs(libsycl-crt SRC crt_wrapper.cpp DEPENDENCIES ${crt_obj_deps})
add_devicelibs(libsycl-complex SRC complex_wrapper.cpp DEPENDENCIES
               ${complex_obj_deps})
add_devicelibs(libsycl-complex-fp64 SRC complex_wrapper_fp64.cpp DEPENDENCIES
               ${complex_obj_deps})
add_devicelibs(libsycl-cmath SRC cmath_wrapper.cpp DEPENDENCIES
               ${cmath_obj_deps})
add_devicelibs(libsycl-cmath-fp64 SRC cmath_wrapper_fp64.cpp DEPENDENCIES
               ${cmath_obj_deps})
add_devicelibs(libsycl-imf SRC imf_wrapper.cpp DEPENDENCIES ${imf_obj_deps})
add_devicelibs(libsycl-imf-fp64 SRC imf_wrapper_fp64.cpp DEPENDENCIES
               ${imf_obj_deps})
add_devicelibs(libsycl-imf-bf16 SRC imf_wrapper_bf16.cpp DEPENDENCIES
               ${imf_obj_deps})
add_devicelibs(libsycl-bfloat16 SRC bfloat16_wrapper.cpp DEPENDENCIES
               ${cmath_obj_deps})
if(MSVC)
...
add_devicelibs(
  ...
  SRC
  fallback-cassert.cpp
  DEPENDENCIES
  ${crt_obj_deps}
  EXTRA_OPTS
  -fno-sycl-instrument-device-code)
add_devicelibs(libsycl-fallback-cstring SRC fallback-cstring.cpp DEPENDENCIES
               ${crt_obj_deps})
add_devicelibs(libsycl-fallback-complex SRC fallback-complex.cpp DEPENDENCIES
               ${complex_obj_deps})
add_devicelibs(libsycl-fallback-complex-fp64 SRC fallback-complex-fp64.cpp
               DEPENDENCIES ${complex_obj_deps})
add_devicelibs(libsycl-fallback-cmath SRC fallback-cmath.cpp DEPENDENCIES
               ${cmath_obj_deps})
add_devicelibs(libsycl-fallback-cmath-fp64 SRC fallback-cmath-fp64.cpp
               DEPENDENCIES ${cmath_obj_deps})
add_devicelibs(libsycl-fallback-bfloat16 SRC fallback-bfloat16.cpp DEPENDENCIES
               ${bfloat16_obj_deps})
add_devicelibs(libsycl-native-bfloat16 SRC bfloat16_wrapper.cpp DEPENDENCIES
               ${bfloat16_obj_deps})
...
set(imf_fallback_fp64_deps
    ...
    imf/imf_fp64_dl.cpp)
set(imf_fallback_bf16_deps
    device.h device_imf.hpp imf_bf16.hpp imf_utils/bfloat16_convert.cpp
...
# For each specified flag, add that flag to 'imf_host_cxx_flags' if the flag is
...
set(obj-new-offload_host_compile_opts ${imf_host_cxx_flags}
...
foreach(datatype IN ITEMS fp32 fp64 bf16)
  ...
  add_custom_command(
    ...
    COMMAND
      ${CMAKE_COMMAND} -D SRC_DIR=${imf_src_dir} -D
      DEST_DIR=${imf_fallback_src_dir} -D IMF_TARGET=${upper_datatype} -P
      ${CMAKE_CURRENT_SOURCE_DIR}/cmake/modules/ImfSrcConcate.cmake
...
#
# Arguments: * SRC <string> ... Source code files needed for the compilation. *
# DIR <string> The directory where the output file should be located in. * FTYPE
# <string> Filetype of the output library file (e.g. 'bc'). * DTYPE <string> The
# datatype of the library, which determines the input source and dependencies of
# the compilation command. * TGT_NAME <string> Name of the new target that
# depends on the compilation of the library. * EXTRA_OPTS <string> ... List of
# extra compiler options to use. Note that the ones specified by the
# compile_opts var are always used.
#
# Depends on the clang target for compiling.
function(add_lib_imf name)
  cmake_parse_arguments(ARG "" "DIR;FTYPE;DTYPE;TGT_NAME" "EXTRA_OPTS" ${ARGN})

  add_custom_command(
    OUTPUT ${ARG_DIR}/${name}.${${ARG_FTYPE}-suffix}
    COMMAND
      ${clang} ${compile_opts} ${ARG_EXTRA_OPTS} -I
      ${CMAKE_CURRENT_SOURCE_DIR}/imf ${imf_${ARG_DTYPE}_fallback_src} -o
      ${ARG_DIR}/${name}.${${ARG_FTYPE}-suffix}
    DEPENDS ${imf_fallback_${ARG_DTYPE}_deps} get_imf_fallback_${ARG_DTYPE}
            sycl-compiler
    VERBATIM)
...
foreach(dtype IN ITEMS bf16 fp32 fp64)
  foreach(ftype IN LISTS filetypes)
    ...
    add_lib_imf(
      ...
      ${dtype}
      EXTRA_OPTS
      ${${ftype}_device_compile_opts}
      TGT_NAME
      ${tgt_name})
  endforeach()
endforeach()

# Add device fallback imf libraries for the NVPTX and AMD targets. The output
# files are bitcode.
foreach(arch IN LISTS devicelib_arch)
  foreach(dtype IN ITEMS bf16 fp32 fp64)
    ...
    add_lib_imf(
      libsycl-fallback-imf-${arch}-${dtype}
      ARCH
      ${arch}
      DIR
      ${bc_binary_dir}
...
foreach(arch IN LISTS devicelib_arch)
  ...
  link_bc(TARGET device_lib_device_${arch} RSP_DIR ${CMAKE_CURRENT_BINARY_DIR}
...
foreach(dtype IN ITEMS bf16 fp32 fp64)
  foreach(ftype IN ITEMS obj obj-new-offload)
    ...
    add_custom_command(
      ...
      COMMAND
        ${clang} ${${ftype}_host_compile_opts}
        ${CMAKE_CURRENT_SOURCE_DIR}/${wrapper_name} -o
        ${${ftype}_binary_dir}/imf-${dtype}-host.${${ftype}-suffix}
      MAIN_DEPENDENCY ${CMAKE_CURRENT_SOURCE_DIR}/${wrapper_name}
      DEPENDS ${imf_obj_deps}
      VERBATIM)

    add_custom_target(
      imf_${dtype}_host_${ftype}
      DEPENDS ${obj_binary_dir}/imf-${dtype}-host.${${ftype}-suffix})
  endforeach()
endforeach()
...
foreach(ftype IN ITEMS obj obj-new-offload)
  ...
  add_custom_command(
    ...
    COMMAND
      ${llvm-ar} rcs ${${ftype}_binary_dir}/${devicelib_host_static_${ftype}}
  ...
  install(
    FILES ${obj_binary_dir}/${devicelib_host_static_${ftype}}
