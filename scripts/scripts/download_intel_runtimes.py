...
if __name__ == "__main__":
    ...
    install_dir = f"{os.environ["ONEAPI_INSTALL_DIR"]}"
    ...
    tbb_cmd = f"ln -sf {dep_install_roots[-1]}/lib/intel64/gcc4.8/*.so* {dep_install_roots[0]}/x64"
    run_cmd(tbb_cmd, dry_run=False)
    tbb_cmd = f"ln -sf {dep_install_roots[-1]}/lib/intel64/gcc4.8/*.so* {dep_install_roots[1]}/x64"
    run_cmd(tbb_cmd, dry_run=False)
